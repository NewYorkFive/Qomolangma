...
@implementation QLMMineEmitterLayer
...
-(void)drawInContext:(CGContextRef)ctx {
    int count = 0;
    for (QLMMineParticle *particle in _particleArray) {
    ...
    if (count == _particleArray.count) {
        [self reset];
        if (_mineEmitterLayerdelegate && [_mineEmitterLayerdelegate respondsToSelector:@selector(mineEmitterLayerAnimEnd)]) {
            [_mineEmitterLayerdelegate mineEmitterLayerAnimEnd];
        }
    }
}
...
//    NSUInteger imageH = CGImageGetHeight(imageRef);
    ...
    for (int y = 0; y < imageH; y+=addY) {
        for (int x = 0; x < imageW; x+=addX) {
            ...
            QLMMineParticle *particle = [QLMMineParticle new];
